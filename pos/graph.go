...
func NewGraph(index, size, pow2 int64, name, fn string, pk []byte) *Graph {
...
func (g *Graph) NewNode(id int64, hash []byte) {
	if id >= 0 {
		...
		num, err := g.merkle.WriteAt(hash, -id*hashSize)
...
func (g *Graph) GetNode(id int64) *Node {
	...
	if id >= 0 {
		...
		num, err := g.merkle.ReadAt(hash, -id*hashSize)
		if err != nil || num != hashSize {
			panic(err)
		}
		node.H = hash
...
func (g *Graph) WriteNode(node *Node, id int64) {
	...
	num, err := g.db.WriteAt(b, id*nodeSize)
	if err != nil || num != nodeSize {
		panic(err)
	}
}
